...
def compute_summary(values: List[float]) -> Optional[float]:  # BUG: annotated Optional, but actually returns float
    if not values:
        return 0.0
    total = sum(values)
...
def display_stats(path: str) -> str:
    ...
    label = format_result(result)   # type error
    return label

def display_secondary_stats(path: str) -> str:
    vals = load_values(path)
    result = compute_summary(vals)
    label = format_result(result)   # type error (identical)
    return label
